...
import { Flame, Salad, Users, Sandwich, MapPin, Phone } from "lucide-react";

export default function Page() {
  return (
    <main className="min-h-screen bg-background text-foreground">
      <MinimalNav
        logo={<span><span style={{ color: "var(--theme-accent)" }}>Bliss</span> Restaurant</span>}
        items={[
      ...
      <StatsSection stats={[{ value: 97, suffix: "+", label: "5-Star Reviews" }, { value: 4.6, label: "Google Rating" }, { value: 6, label: "Days a Week" }, { value: 9, suffix: "hrs", label: "Daily Service" }]} />
      ...
      <section id="menu" className="py-20 md:py-28 bg-card">
        <div className="max-w-6xl mx-auto px-6">
          <SectionHeading title="Our Menu" subtitle="Fresh Mediterranean flavors, made daily" />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mt-12">
            {content.services.map((item: any, i: number) => {
              const icons = [<Flame key="f" className="w-8 h-8" />, <Salad key="sa" className="w-8 h-8" />, <Users key="u" className="w-8 h-8" />, <Sandwich key="sw" className="w-8 h-8" />];
              return (
      ...
      <section id="reviews" className="py-20 md:py-28">
        <SectionHeading title="What Our Guests Say" subtitle="97+ five-star reviews" />
        <div className="mt-12">
          <TestimonialMarquee testimonials={content.reviews.map((r: any) => ({ quote: r.text, author: r.name, rating: r.rating }))} />
        </div>
      </section>

      <CTABanner heading="Ready for Mediterranean Bliss?" subheading="Dine in or call ahead for takeout — fresh kebabs and meze waiting for you." ctaText="Call Now" ctaHref="tel:8185000008" variant="filled" />
          ...
          <div>
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <a href="tel:8185000008" className="text-muted-foreground hover:underline">{content.contact.phone}</a>
              </div>
            </div>
      ...
      <BookingPrompt heading="Catering Available" description="Planning an event? Call us for custom catering menus and family platters." phone={content.contact.phone} ctaText="Call Now" />
